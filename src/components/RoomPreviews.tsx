...
export default function RoomPreviews() {
  ...
  useEffect(() => {
    const els = ref.current?.querySelectorAll(".room-preview-card");
    if (!els) return;
    const obs = new IntersectionObserver(
      (entries) => entries.forEach((e) => {
        if (e.isIntersecting) e.target.classList.add("room-preview-visible");
      }),
      { threshold: 0.15 }
    );
  ...
  return (
    <section className="room-previews" ref={ref}>
      {rooms.map((room) => (
