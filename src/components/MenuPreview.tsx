  ...
  return (
    <section className="menu-preview" id="menu-preview">
      ...
      <div className="menu-pdf-buttons">
        <a href="/Kod1847_Tea_Menu.pdf" target="_blank" rel="noopener noreferrer" className="menu-pdf-btn" onClick={() => trackEvent("click_pdf", { menu: "tea" })}>{t("Чайная карта", "Tea Menu")}</a>
        <div className="menu-pdf-divider" />
        <a href="/Kod1847_Hookah_Menu2.pdf" target="_blank" rel="noopener noreferrer" className="menu-pdf-btn" onClick={() => trackEvent("click_pdf", { menu: "hookah" })}>{t("Кальянная карта", "Hookah Menu")}</a>
      </div>
      <button className="menu-reserve-btn" onClick={() => { trackEvent("click_reserve", { location: "menu_preview" }); onReserve(); }}>
